...
function excelread() {
	readXlsxFile('Clubes.xlsx').then((rows: string | any[]) => {
...
function dividaramal(ramalref: string, index: number, rows: any) {
...
function lerarquivoporfavor(ramalref2: string, index2: any, rows2: any[][]) {

	let dataBuffer = fs.readFileSync(ramalref2 + '.pdf')
	pdf(dataBuffer).then(function (data: { text: { toString: () => any; }; }) {

		//console.log(data);
		let textRef = data.text.toString();
...
function lerarquivoporfavor2(ramalref2: string, index2: any, rows2: any[][]) {
					...
					if (ano == mestostring.getAno()) {
						...
						if (mes >= mestostring.getMes() - 1) {
							...
							if (mes == mestostring.getMes()) {
								...
								if (competenciaValue == mestostring.getComp(1)) {
									if (dia >= grupoRef || mes == mestostring.getMes()-1) {
										valortotal = valortotal + totalRef;
										console.log(aguaservRef);
										console.log(esgotoRef);
										console.log(correcaoRef);
	...
	//return vencidaTotal;
	fs.unlink(ramalref2 + '.pdf', function (err: any) {
		if (err) throw err;
